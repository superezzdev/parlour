...
import { ServiceCategory, Service } from '@/data/services'
...
function ServiceRowItem({
  ...
  const rowRef = useScrollReveal<HTMLElement>({ delay: 0.1, y: 35 })
  const isReversed = serviceIdx % 2 === 1
  const serviceIndexNumber = `${categoryNumber}.${serviceIdx + 1}`
      ...
      <div className={styles.contentColumn}>
        <div className={styles.serviceMeta}>
          <span className={styles.serviceTag}>
            {service.categorySlug.toUpperCase()} · DISC. {categoryNumber}
          </span>
        </div>
...
export default function ServiceEditorialSection({
  ...
  const taglineRef = useScrollReveal<HTMLParagraphElement>({ y: 30, delay: 0.1 })
        ...
        <div className={styles.categoryHeader}>
          ...
          <div className="overflow-hidden">
            <EditorialHeading
              ...
              className={`${styles.categoryTitle} section-heading`}
            >
              {category.name}
            </EditorialHeading>
          </div>

          <p ref={taglineRef} className={styles.categoryTagline}>{category.description}</p>
        </div>
